// Frontend/src/Components/Home/KoreanHindiSection.js
...
const BROWSE_VALUES = ['Korean (Hindi Dubbed)'];

// Simple in-memory cache
const koreanHindiCache = {
  movies: [],
  error: null,
  loaded: false,
};

function KoreanHindiSection() {
  const [movies, setMovies] = useState(koreanHindiCache.movies || []);
  const [loading, setLoading] = useState(!koreanHindiCache.loaded);
  const [error, setError] = useState(koreanHindiCache.error || null);

  const prevEl = useRef(null);
  const nextEl = useRef(null);

  useEffect(() => {
    if (koreanHindiCache.loaded) {
      return;
    }
    ...
    (async () => {
      try {
        ...
        koreanHindiCache.movies = merged;
        koreanHindiCache.error = null;
        koreanHindiCache.loaded = true;
      } catch (e) {
        const message = e?.message || 'Failed to load Korean Hindi section';
        if (!cancelled) {
          setError(message);
          setLoading(false);
        }
        koreanHindiCache.error = message;
        koreanHindiCache.loaded = false;
      }
    })();
  ...
  return (
    <div className="my-8 mobile:my-4">
      ...
      {showLoader ? (
        ...
        <Empty message="No titles found in Korean Hindi" />
      )}
    </div>
  );
}

export default KoreanHindiSection;
