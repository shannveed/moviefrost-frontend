...
function Korean() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e?.message || 'Failed to load Korean titles');
      } finally {
  ...
  return (
    <Layout>
      <MetaTags
        title="Korean Movies & Web Series | MovieFrost"
        description="Watch Korean movies and web series online for free in HD. Including Korean (English) and Korean Drama (Korean)."
        keywords="Korean movies, Korean web series, free HD streaming"
        url="https://www.moviefrost.com/Korean"
      />
      <div className="container mx-auto px-8 mobile:px-4 my-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl font-bold">Korean</h1>
        ...
        {loading ? (
          ...
          <div className="w-full gap-6 flex-colo min-h-[50vh]">
            <Empty message="No Korean titles found" />
          </div>
        ) : (
          <>
