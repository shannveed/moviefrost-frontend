...
function JapaneseAnime() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e?.message || 'Failed to load Japanese Anime titles');
      } finally {
  ...
  return (
    <Layout>
      <MetaTags
        title="Japanese Anime | MovieFrost"
        description="Watch your favorite Japanese Anime series & movies online free in HD."
        keywords="Japanese Anime, Anime Series, free HD streaming"
        url="https://www.moviefrost.com/japanese-anime"
      />
      <div className="container mx-auto px-8 mobile:px-4 my-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl font-bold">Japanese Anime</h1>
          <span className="text-sm text-border">Showing {totalLoaded} items</span>
        </div>

        {loading ? (
          <div className="w-full gap-6 flex-colo min-h-[50vh]"><Loader /></div>
        ) : error ? (
          <Empty message={error} />
        ) : items.length === 0 ? (
          <Empty message="No Japanese Anime titles found" />
        ) : (
          <>
            <div className="grid sm:mt-6 mt-4 xl:grid-cols-5 above-1000:grid-cols-5 2xl:grid-cols-5 lg:grid-cols-3 sm:grid-cols-2 mobile:grid-cols-2 grid-cols-1 gap-4 mobile:gap-2">
              {items.map((movie) => <Movie key={movie._id} movie={movie} />)}
            </div>
