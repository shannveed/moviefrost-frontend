...
function Chinese() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e?.message || 'Failed to load Chinese titles');
      } finally {
  ...
  return (
    <Layout>
      <MetaTags
        title="Chinese | MovieFrost"
        description="Watch Chinese dramas online for free in HD."
        keywords="Chinese dramas, Chinease Drama, free HD streaming"
        url="https://www.moviefrost.com/Chinese"
      />
      <div className="container mx-auto px-8 mobile:px-4 my-6">
        ...
        {loading ? (
          ...
          <div className="w-full gap-6 flex-colo min-h-[50vh]">
            <Empty message="No Chinese titles found" />
          </div>
        ) : (
          <>
